            ...
            CBUFFER_END
                ...
                UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
                    ...
                    UNITY_DOTS_INSTANCED_PROP(float, _ShadowReceiveBlur)
                ...
                #define _ShadowReceiveBlur UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float, _ShadowReceiveBlur)
                #define _Fresnel UNITY_ACCESS_DOTS_INSTANCED_PROP_WITH_DEFAULT(float, _Fresnel)
            ...
            float3 generateToonLightBlinnPhong(float3 N, float3 wPos, float3 V,Light light, float4 ambientColor, float gloss, float rimSize, float rimBlur, float rimThreshold){
                ...
                specularLight = smoothstep(0.005,0.006,specularLight)*_GlossIntensity;

                fresnel = lerp(fresnel,_Fresnel,(1-fresnel));
                fresnel = _Fresnel >0? fresnel: 1; 

                float shadowAttenuation =  smoothstep(_ShadowReceiveBlur,_ShadowReceiveBlur+0.01,light.shadowAttenuation);
                //return float4(lambert.xxx,1) ;
                float3 Col = LightColor*( 
                deffuseLight *shadowAttenuation
                + specularLight*shadowAttenuation
                + ambientColor.rgb 
                + rim*shadowAttenuation )*fresnel;
            ...
            half4 frag(vertOut i, FRONT_FACE_TYPE frontFace : FRONT_FACE_SEMANTIC) : SV_Target//get front face of object
            {
                ...
                return CalculateFinalColor(lightingData,_Color.a);
            }